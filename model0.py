...
def ResCBAM1(in_image=(height, width, 1)):
    ...
    x0 = Conv2D(32, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv0_0')(img_in_b)
    y0 = Conv2D(32, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv0_1')(img_in_b)
    y0 = Conv2D(32, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv0_2')(y0)
    y0 = attach_attention_module(y0, attention_module='cbam_block')
    conv0= add([x0,y0]) #32
    
    x1 = conv0
    y1 = Conv2D(32, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv1_1')(conv0)
    y1 = Conv2D(32, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv1_2')(y1)
    y1 = attach_attention_module(y1, attention_module='cbam_block')
    
    conv1= add([x1,y1])#32
    
    x2 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv2_0')(conv1)
    y2 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv2_1')(conv1)
    y2 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv2_2')(y2)
    y2 = attach_attention_module(y2, attention_module='cbam_block')
    conv2= add([x2,y2])#64
  
    x3 =conv2
    y3 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv3_1')(conv2)
    y3 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv3_2')(y3)
    y3 = attach_attention_module(y3, attention_module='cbam_block')
    conv3= add([x3,y3]) #64
    
    x4 = conv3
    y4 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv4_1')(conv3)
    y4 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv4_2')(y4)
    y4 = attach_attention_module(y4, attention_module='cbam_block')
    conv4 = add([x4,y4])
    
    x5 = conv4
    y5 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv5_1')(conv4)
    y5 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv5_2')(y5)
    y5 = attach_attention_module(y5, attention_module='cbam_block')
    conv5 = add([x5,y5])
    
    x6 = conv5
    y6 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv6_1')(conv5)
    y6 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv6_2')(y6)
    y6 = attach_attention_module(y6, attention_module='cbam_block')
    conv6 = add([x6,y6])
    
    x7 = conv6
    y7 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv7_1')(conv6)
    y7 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal',name='conv7_2')(y6)
    y7 = attach_attention_module(y7, attention_module='cbam_block')
    conv7 = add([x7,y7])
    
    segmentation = Conv2D(1, 1, activation = 'sigmoid', name='segmentation')(conv7)
    
    pool1 = AveragePooling2D(pool_size=8,name='AvePool1' )(conv7)
    down_1 = Flatten(name='down_1')(pool1)
    classification = Dense(2,activation='sigmoid',name='classification')( down_1)
